//! The [`Runtime`] seam plus the `--demo` engine. [`DemoScript`] /
//! [`ScriptedDemoRuntime`] are the port of Python `kernel/demo.py`'s
//! `DemoRuntime`: the mockup's rich multi-turn demo (seed / build / auto /
//! plan / brainstorm / agents) as deterministic virtual-clock event scripts,
//! selected per prompt through the [`DemoWiring`] key bookkeeping. It emits
//! the SAME wire vocabulary the protocol backend does (`kernel/events.py`
//! kinds plus the ticket-bearing approval record), on a background thread,
//! so streaming/approvals are genuinely time-sliced. No network, no Python.
//! Reachable ONLY via the explicit `--demo` flag (Python parity).
...
/// The runtime seam — the one interface the UI talks to.
/// [`ScriptedDemoRuntime`] (scripted) and `CoreClientRuntime` (spawned
/// backend) both implement it, so the app is identical whether events come
