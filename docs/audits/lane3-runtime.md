...
**Target:** `michaeljabbour/amplifier-app-tui` @ e6b50cd → `src/amplifier_app_tui/kernel/`.
**Method:** capability parity, not code parity. All paths are repo-relative to each package.
Verdicts: PARITY | PARTIAL | MISSING | TUI-BETTER | N/A-BY-DESIGN.

Structural note: app-cli is a prompt_toolkit line-mode REPL; tui is a full-screen
...
tui owns the screen and normalizes every hook payload into a `UIEvent` queue
...
| Capability | app-cli cite | tui cite / status | Verdict | Sev | Recommendation |
|---|---|---|---|---|---|
...
| `overrides.<id>.config` deep-merge (providers/tools/hooks + agents) | `lib/settings.py:1163-1170`; `runtime/config.py:195-216` | `kernel/config.py:346-362` (root sections; agents not walked) | PARTIAL | Low | app-cli also applies config overrides to agent-scoped declarations (`runtime/config.py:205-215`); tui applies to root sections only |
...
| `${VAR}`/`${VAR:default}` expansion (+ drop-empty fail-safe) | `runtime/config_merge.expand_env_vars`; `runtime/config.py:318` | `kernel/config.py:377-425,864` | TUI-BETTER | — | tui drops an exactly-one-unset `${VAR}` instead of expanding to `""` (avoids empty base_url) |
| `keys.env` load at startup (exported env wins) | `key_manager.KeyManager._load_keys` | `kernel/config.py:218-243,807` | PARITY | — | — |
| `context.max_tokens`/`compact_threshold`/`auto_compact` settings keys | inherited from bundle; no dedicated keys | `kernel/config.py:849`; `kernel/compaction.py:139-194` | TUI-BETTER | — | first-class validated context knobs |
| `pricing.live` + on-disk pricing cache | none (in-memory estimator only, `cost_history.py`) | `kernel/cost.py:462-588` | TUI-BETTER | — | 24h cached Helicone table, new-turns-only swap |
| `permissions.write_boundary` (open/guarded governance gate) | allowed/denied paths only | `kernel/directory_permissions.py`; `docs/SETTINGS.md:57` | TUI-BETTER | — | app-level write boundary w/ H2 auto-degrade |
| `resume.use_active_bundle` | resume reattaches under stored bundle (implicit) | `docs/SETTINGS.md:59` | TUI-BETTER | — | explicit switch + boot notice |
...
| Malformed-settings resilience (skip, never block boot) | `lib/settings.py:135-136` (silent) | `kernel/config.py:151-198` + user-facing notice | TUI-BETTER | — | surfaces the skipped scope instead of a silent warning |
...
| `routing.enabled` explicit on/off switch | none | `kernel/config.py:279-294` | TUI-BETTER | — | — |
| routing-instructions context + skills mounted with hook | not added at session start (config-override only) | composed whole bundle `kernel/config.py:267-272,306-308` | TUI-BETTER | — | — |
...
| Fork / rewind (checkpoint-addressed, confirm-then-trim) | `ui/outcome_ledger.py`, `ui/turn_outcomes.py`; foundation `fork_session` | `kernel/rewind.py:75-213` | PARITY | — | tui addresses by checkpoint id (never label) |
...
| Per-lane steering (steer a delegate at its own boundary) | not present | `kernel/steering.py:99-123` (issue #39) | TUI-BETTER | — | — |
...
| Compaction / context management | `runtime/config_behaviors.py`; context module | `kernel/compaction.py:38-194`; provider-observed vs estimated accounting | PARITY | — | tui adds accounting-mode reporting |
...
| Credential env detection table | `provider_env_detect.py:9-50` | `kernel/setup.py:310-385` | PARITY | Low | app-cli gates on installed entry_points + ollama-last; tui table-only (functionally equivalent) |
...
| Update/upgrade of composed bundles/modules | `commands/update.py` (WELL_KNOWN + uv self-update) | `kernel/updater.py:75-150` (foundation `check_bundle_status`/`update_bundle`) | PARITY | — | app self-update is a hint (`self_update_hint:144-150`) since tui isn't the umbrella tool — correct by design |
...
- **tui** opts in via `routing_enabled()` (`kernel/config.py:279-294`): `routing.enabled`
  (bool) **wins**, else a non-empty `routing.matrix` string. **Any other shape ⇒ off.** So an
  **overrides-only** settings block does **not** enable routing on tui — the overrides are
  ...
  the matrix/enabled contract clearly). In exchange tui adds an explicit `routing.enabled`
  kill/enable switch app-cli lacks.

**Which keys does tui bridge vs app-cli?** Equivalent set, same precedence:
...
does **not** add routing instructions/skills at session start. tui instead **composes the
...
on both sides. tui reaches the same list/fetch behavior through the **shared foundation
...
app-cli but is inert on tui. tui is otherwise TUI-BETTER (explicit `routing.enabled`;
...
3. **`config.notifications.*` field-level config keys ignored** (PARTIAL, Med) — desktop
   ...
   (`lib/settings.py:581-677`); tui exposes only env gates + bundle config. Common on/off is
...
4. **Overrides-only routing opt-in diverges** (PARTIAL, Low) — a `routing:` block with only
   `overrides` opts in on app-cli, is inert on tui (`kernel/config.py:279-294`). → Either
...
- Whether tui's `commands/` layer resolves an added-bundle name→URI *before* writing
...
`config.tui.startup_*` all exist to survive app-cli's shared-stdout REPL. tui owns the
