...
**Question:** does `michaeljabbour/amplifier-app-tui` reproduce every user-facing
...
`ui/command_processor.py`), then matched each to tui's CLI groups (`main.py`),
...
Legend — verdict: **PARITY** / **PARTIAL** / **MISSING** / **TUI-BETTER** /
...
| `run --output-format` (text/json/…) | `run.py:59-64` | `main.py:274-291` (text/json/json-trace/**jsonl** live event stream) | TUI-BETTER | — | tui adds versioned JSONL streaming; keep |
...
| `session rename` | *(no group subcmd; /rename slash only)* `core_commands`→`command_processor` | `main.py:418-430` (`session rename`) | TUI-BETTER | — | tui exposes rename at CLI too |
...
| `provider use` (set primary) | *(priority via edit/manage)* `provider.py:770,1583` | `main.py:991-1001` | TUI-BETTER | — | explicit `use` is clearer than app-cli priority editing |
...
| `init` provider/routing setup | `commands/init.py:399-420`, dashboard `:276-397` | `main.py:920-939`, `_init:829-917` | PARITY | — | tui init = provider-key setup; routing via `routing use` |
...
| `doctor` standalone setup checkup | *(no standalone; `/doctor` session only)* | `main.py:479-484` + `commands/doctor.py` | TUI-BETTER | — | tui adds a non-session `doctor` |
| `version` | `commands/version.py` (`main.py:455`) | `--version` `main.py:258` + `/about` `builtin.py:408` | PARITY | — | none |
| `--install-completion` (bash/zsh/fish) | `main.py:132-197`, `commands/completion.py` | *(none)* | MISSING | L | add shell-completion install for tui CLI |
...
| `module validate` (contract + behavioral tests) | `module.py:545-664` | *(none)* | MISSING | L | module-author dev tool; likely out of tui user scope |
...
| `/export [md|json]` | `catalog:179-186`, `core_commands.py:452` | `builtin.py:199-202,384`; `commands/export.py` | PARITY | — | tui md-only; JSON export is `run --output-format json` |
...
| Capability | tui cite | app-cli | Verdict |
|---|---|---|---|
| `/theme` runtime UI theme (slate/graphite/carbon) | `builtin.py:235-237,492`; `ui/themes.py` | none | TUI-BETTER |
| `/copy` last answer → clipboard (OSC 52) | `builtin.py:205-212,392`; `commands/copy.py` | none (no slash) | TUI-BETTER |
| `/about` app/core/bundle/session identity block | `builtin.py:215-226,408` | `version` CLI only | TUI-BETTER |
| `/plan`, `/brainstorm` promoted to top-level verbs | `builtin.py:49-56,267-280` | via `/mode plan` only | TUI-BETTER (discoverability) |
| Phase-grouped fuzzy command palette (During/Parallel/Ship/Between/Repair) | `registry.py:37-44,431-444` | flat `/help` text | TUI-BETTER |
| Open command registry (skills/recipes contribute verbs at runtime) | `registry.py:319-405` | fixed builtin catalog | TUI-BETTER |
| `run --output-format jsonl` live versioned event stream | `main.py:89-161` | text/json/json-trace only | TUI-BETTER |
| `doctor` as standalone (non-session) CLI | `main.py:479-484` | session `/doctor` only | TUI-BETTER |
| `session rename` / `provider use` at CLI level | `main.py:418,991` | slash / priority-edit only | TUI-BETTER |
...
1. **`tool invoke` (CLI) — MISSING (M).** No way to invoke a mounted bundle tool
   directly from the shell (`amplifier tool invoke read_file path=…`); tui only
...
5. **`run` per-invocation overrides + one-shot `--resume` — PARTIAL (M).** `run` can't
   ...
   are core scripting knobs on app-cli. → add the flags to tui `run`.
