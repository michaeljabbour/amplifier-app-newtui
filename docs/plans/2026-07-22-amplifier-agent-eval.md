...
- **Bundle control is a hard requirement.** The TUI is *"a THIN WRAPPER"* that **suppresses printing hooks and OSC/BEL `hooks-notify` at boot** (built-in suppression list + `hooks.suppress`) and keeps `hooks-logging` native. The former mounted `hooks-notify-push` path is historical; the current app-owned, stdout-free ntfy sink consumes normalized attention events, and the legacy hook is always suppressed if an overlay brings it back. `AGENTS.md:26` restates the non-negotiable: *"Never mount printing hooks … they write ANSI to stdout and corrupt the Textual screen."* Overlay + suppression + roster control are load-bearing, not incidental.
