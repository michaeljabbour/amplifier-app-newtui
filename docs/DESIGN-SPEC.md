...
- [ ] **Tool line (collapsed)**: `  ● <summary> ` in dim + `· click to expand` in dimmer. Click/enter toggles an indented dimmer body line below. One dim line per tool batch (e.g. `Ran 2 shell commands`).
