"""Regression guard: the packaged newtui bundle composes ``tool-team-pulse``,
which does ``from team_pulse_lib import ...`` at mount(). ``team_pulse_lib`` is
an unpublished sibling package of the team-pulse bundle; newtui pulls only the
tool-module subdirectory, so nothing installs the lib unless newtui declares it
...
that made that bug possible — the lib is importable in newtui's environment.

If team-pulse is ever dropped from ``data/bundles/newtui.md``, drop this test
...
def test_team_pulse_lib_importable() -> None:
    assert importlib.util.find_spec("team_pulse_lib") is not None, (
        "team_pulse_lib missing — tool-team-pulse (composed by the packaged newtui "
