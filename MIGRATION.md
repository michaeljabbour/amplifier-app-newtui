...
| backend resolution (real session by default) | main.py `_interactive_launch` | main::test_launch_flags_assemble_backend_command (resolve_backend arms: env override / checkout / mock fallback) | verified | `resolve_backend`: `AMPLIFIER_SERVE_CMD` wins outright; inside the checkout spawns the REAL `uv run --project <checkout> amplifier-newtui serve`; otherwise the offline `backend/serve_mock.py` with an explicit fallback notice |
