...
Ported from amplifier-app-cli ``ui/git_yield.py`` (the reference
implementation for real-runtime turn outcomes). The RealRuntime captures
one snapshot before and one after each turn; the delta between them is
the turn's concrete yield (``files N`` / ``+A/−D`` on the turn rule —
DESIGN-SPEC §3 shipped outcomes).

Kernel-pure: asyncio + git subprocess only, no Textual, no amplifier-core.
"""
...
async def capture_git_diff(cwd: Path, *, timeout_seconds: float = 5.0) -> GitDiffSnapshot:
