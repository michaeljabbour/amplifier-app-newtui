...
1. **Posture** — the single shift+tab trust baseline (chat/plan/brainstorm/
   build/auto, :mod:`amplifier_app_newtui.model.modes` +
   :mod:`amplifier_app_newtui.model.trust`). What capabilities auto-run/ask/
...
So newtui models an ordered **stack** of active native modes client-side. The
**primary** (most-recently activated, the top of the stack) is what newtui
...
- The kernel governance hook lets a tool the active native mode declares
  ...
  :class:`amplifier_app_newtui.kernel.governance_hook.GovernanceHook`.)
...
class ActiveNativeModes(BaseModel):
    ...
    the one newtui points the upstream single slot at.
    """
...
def posture_restricts_tools(posture_id: str) -> bool:
    ...
    Derived from :mod:`amplifier_app_newtui.model.trust` rather than a
