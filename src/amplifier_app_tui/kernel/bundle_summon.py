...
1. **Discovery** — :func:`build_deferred_catalog` names each held-back overlay
   ...
   :class:`~amplifier_app_tui.kernel.surface_hint.SurfaceHintInjector` (so it
...
def build_deferred_catalog(
    ...
    file resolution (via :func:`~amplifier_app_tui.kernel.config.discover_bundle`)
...
class DeferredCatalogInjector:
    ...
    Mirrors :class:`~amplifier_app_tui.kernel.surface_hint.SurfaceHintInjector`:
